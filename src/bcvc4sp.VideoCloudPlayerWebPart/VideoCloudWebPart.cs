...
namespace BrightcoveVideoCloudPlayer
{
    ...
    public class VideoCloudWebPart : WebPart
    {
        ...
        private void GetConfigurationfromList()
        {
            ...
            {
                siteSelected = Context.Request.QueryString["Site"];
            }
            else
            {
                siteSelected = SPContext.Current.Web.Url;
            }

            try
            {
                using (SPSite site = new SPSite(siteSelected))
                {
                    SPWeb web = site.OpenWeb();
                    SPList list = web.GetList(siteSelected + "/" + accountListAddress);
